...
export default {
  ...
  [INDENT_GOODS_LIST](state, {indents}) {
  ...
  [DEL_PATCH_GOODS](state, {goods}){
    goods.forEach(item => {
      item.buy_count = 0
    });
    const ids = goods.map(item => item.goods_id)
    state.cartgoods = state.cartgoods.filter(item => !ids.includes(item.goods_id))
  },
